PWAYS_INTEGRATION_CELLLINES = function(Output_file_path,Target_list_path, ORGANISM = 'Mouse', DirPipeline_Data, Control ){
    ...
    for(mol in Mols){
        ...
        NamesDiff_First = sapply(names(DiffGenes), function(x){str_split(x, '_')[[1]][1]})
        DiffGenes = DiffGenes[grep(mol, NamesDiff_First)]
        DiffGenes_filt = list()
    ...
    Diff_Genes_Tot[is.na(Diff_Genes_Tot)] = 0
    ...
    # PLOT SUMMARY FOR DEGs
    DEG_summ = data.frame(Mol = character(), dose = character(), time = character(), stim = character(), n_diff = numeric())
    Comps = colnames(Diff_Genes_Tot)[colnames(Diff_Genes_Tot)!= 'Gene']

    for(x in Comps){

        Mol = str_split(x, '_')[[1]][1]
        dose =  str_split(x, '_')[[1]][2]
        time = str_split(x, '_')[[1]][3]
        stim = str_split(x, '_')[[1]][4]

        n_diff = length(which(Diff_Genes_Tot[[x]] !=0))

        DEG_summ = DEG_summ %>% add_row(Mol = Mol, dose =dose, time = time, stim = stim, n_diff =n_diff)

    }


    save(DEG_summ, file = "Summary_Plot_DEG.RData")
