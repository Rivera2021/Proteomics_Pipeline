...
# Getting read of the part where an external data base is referenced to find number of detected genes
QC_PRENORMALIZATION_CL_V2 = function(Output_file_path,nMust = 2, TPM_path){
    library(tidyverse)
    library(dplyr)
    library("stringr")
    library("openxlsx")
    ...
    p4 = ggplot(Metadata, aes(x=ReadNum, y=detected_genes, group = Treatment)) +
        ...
        xlab("Number of reads") +
        ylab("Number of non-zero genes")+
        ggtitle(paste("Number of reads vs Non-zero genese. Dashed: Q1 - ", as.character(nMust), "x IQR",sep = " "))+
        theme(plot.title = element_text(size = 15))
